...
static void *leftEyeTexturePtr1 = nullptr;
static void *rightEyeTexturePtr1 = nullptr;
static void *leftEyeTexturePtr2 = nullptr;
static void *rightEyeTexturePtr2 = nullptr;
...
static std::vector<FrameInfo> frameInfo(2);
size_t iteration = 0;
...
int ConstructBuffersD3D11(int eye, int frame);
...
LinkDebug(void(_stdcall *d)(const char *)) {
  debugLog = d;
}
}
...
ShutdownRenderManager() {
  ...
  if (render != nullptr) {
    delete render;
    render = nullptr;
    rightEyeTexturePtr1 = nullptr;
    leftEyeTexturePtr1 = nullptr;
  }
}
...
void SetRoomRotationUsingHead() { render->SetRoomRotationUsingHead(); }
...
void ClearRoomToWorldTransform() { render->ClearRoomToWorldTransform(); }

// Called from Unity to create a RenderManager, passing in a ClientContext
extern "C" OSVR_ReturnCode UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API
...
extern "C" osvr::renderkit::OSVR_ViewportDescription
    UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API
    GetViewport(int eye) {
  return renderInfo[eye].viewport;
}

extern "C" osvr::renderkit::OSVR_ProjectionMatrix
    UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API
    GetProjectionMatrix(int eye) {
  return renderInfo[eye].projection;
}

extern "C" OSVR_Pose3 UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API
GetEyePose(int eye) {
...
void RenderViewD3D11(const osvr::renderkit::RenderInfo &renderInfo,
  ...
  frameInfo[frame].renderBuffers[eyeIndex].D3D11->colorBuffer =
      eyeIndex == 0 ? reinterpret_cast<ID3D11Texture2D *>(leftEyeTexturePtr1)
                    : reinterpret_cast<ID3D11Texture2D *>(rightEyeTexturePtr1);
}
...
void RenderViewOpenGL(
  ...
  GLuint glTex = eyeIndex == 0 ? (GLuint)leftEyeTexturePtr :
  (GLuint)rightEyeTexturePtr;
...
SetColorBufferFromUnity(void *texturePtr, int eye, int frame) {
  if (s_DeviceType == -1)
    return OSVR_RETURN_FAILURE;
  ...
  if (eye == 0) {
    if (frame == 0) {
      leftEyeTexturePtr1 = texturePtr;
    } else {
      leftEyeTexturePtr2 = texturePtr;
    }

  } else {
    if (frame == 0) {
      rightEyeTexturePtr1 = texturePtr;
    } else {
      rightEyeTexturePtr2 = texturePtr;
    }
  }
  ...
  case kOsvrEventID_Render:
    if (s_DeviceType == kUnityGfxRendererD3D11) {
      ...
      for (size_t i = 0; i < renderInfo.size(); i++) {
        ...
        // renderBuffers[i].D3D11->colorBufferView, i);
        RenderViewD3D11(
            renderInfo[i],
            frameInfo[frame].renderBuffers[i].D3D11->colorBufferView, i, frame);
      }

      // Send the rendered results to the screen
      // Flip Y because Unity RenderTextures are upside-down on D3D11
      if (!render->PresentRenderBuffers(
              frameInfo[frame].renderBuffers, renderInfo,
              osvr::renderkit::RenderManager::RenderParams(),
              std::vector<osvr::renderkit::OSVR_ViewportDescription>(), true)) {
    ...
    //@todo OpenGL path is not working yet
      ...
      renderBuffers[i].OpenGL->colorBufferName,i);
      }

      // Send the rendered results to the screen
      if (!render->PresentRenderBuffers(renderBuffers, renderInfo)) {
              DebugLog("PresentRenderBuffers() returned false, maybe because it
      was asked to quit");
      }*/
    }
    break;
